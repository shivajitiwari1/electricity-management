...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.bill.count({
      where: {
        billDate: { gte: new Date(now.getFullYear(), now.getMonth(), 1) },
      },
    }),
    ...
    prisma.bill.findMany({
      ...
      include: {
        connection: { include: { resident: { include: { user: true } } } },
      },
    }),
  ]);
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: `₹${Number(revenue).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      icon: IndianRupee,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Link href="/admin/meter-readings">
          <Button variant="outline">Enter Reading</Button>
        </Link>
                  ...
                  recentBills.map((bill) => (
                    <tr key={bill.id} className="border-b last:border-0 hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3">
                        <Badge className={getBadgeClass(bill.status)}>
                          {bill.status}
                        </Badge>
                      </td>
                    </tr>
