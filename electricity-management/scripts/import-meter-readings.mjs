// Import June 30, 2026 meter readings from SOCIETY ELECTRICITY WORKING 15-07-26 (4).xlsx
// Reads the 485-INVENTORY sheet and creates MeterReading records for all matched connections.
...
const READING_DATE = new Date("2026-06-30T00:00:00.000Z");

// Strip parenthetical suffixes like "(Meter Change)", "(APR-MISSING)", "(12883+519)"
...
async function main() {
  ...
  inv.slice(1).forEach((row) => {
    ...
    const may = Number(row[5]) || 0;
    const june = Number(row[7]) || 0;
    const diffMayJune = Number(row[8]) || 0;
    if (june <= 0) return; // skip zero/missing June readings
    rows.push({ flat, normalFlat: normalizeFlat(flat), may, june, diffMayJune });
  });
  console.log(`Found ${rows.length} rows with valid June readings`);

  // Load all DB connections into a map
  const dbConns = await prisma.connection.findMany({ select: { id: true, flatNo: true } });
  const dbMap = new Map(dbConns.map((c) => [c.flatNo, c.id]));
  console.log(`DB has ${dbConns.length} connections`);

  // Get admin user
  const admin = await prisma.user.findFirst({ where: { role: "ADMIN" }, select: { id: true } });
  if (!admin) throw new Error("No admin user found");

  // Check for existing readings on this date to avoid duplicates
  ...
  if (alreadyImported.size > 0) {
    console.log(`Skipping ${alreadyImported.size} connections that already have a June-30 reading`);
  }

  console.log("\n=== Importing June 30, 2026 meter readings ===");
  ...
  for (const row of rows) {
    ...
    // Calculate units — if May is 0, units = June reading itself
    const ncplUnits = row.may > 0 ? row.june - row.may : row.june;
    ...
    try {
      await prisma.meterReading.create({
        data: {
          ...
          ncplPrevious: row.may,
          ncplCurrent: row.june,
